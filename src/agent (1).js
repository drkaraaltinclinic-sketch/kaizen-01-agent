 ...
 * CONSTITUTION v2: Kaizen may APPLY Tier-A remedies itself — env-var changes to
 * variables VERIFIED to exist (kaizen-executor registry), never locked vars,
 * max 2/day with 8h cooldown. Duplicates are auto-rejected. Everything else
 * still waits for a human ACCEPT — which now also auto-applies via Railway.
 */
...
${brain.ledgerPromptBlock()}
${outcomesPromptBlock(await brain.fetchOutcomes().catch(() => null))}
...
Propose 1-4 NEW improvement tasks. Quality over quantity — an empty list is acceptable if nothing genuine emerges. Prefer ENV_VAR remedies (tunable without code). Return ONLY JSON, no fences:
{
 ...
 "tasks": [{
   ...
   "effort": "ENV_VAR|SMALL_PATCH|NEW_MODULE"
    ...
    // New tasks run the DECISION LAYER: duplicates auto-rejected, Tier A remedies
    // auto-applied by the executor (within budget), the rest held for review.
    for (const nt of (out.tasks || []).slice(0, 4)) {
      state.taskSeq++;
      const task = { id: state.taskSeq, ...nt, status: 'PROPOSED', createdAt: new Date().toISOString() };
      const d = decide(task, { ledger: buildLedger(state.tasks), tasks: state.tasks });
      if (d.verdict === 'AUTO_REJECT') {
        task.status = 'REJECTED'; task.rejectReason = d.reason;
      } else if (d.verdict === 'AUTO_APPLY') {
        const r = await executor.applyRemedy(task, fetch);
        if (r.ok) {
          task.status = 'DEPLOYED';
          task.autoAppliedAt = new Date().toISOString();
          task.deployedAt = task.autoAppliedAt;
          task.applyNote = r.note;
        } else { task.applyNote = `auto-apply refused: ${r.note}`; }
      } else { task.applyNote = d.reason; }
      state.tasks.unshift(task);
      emit('TASK', 'kaizen.task', task);
      emit('SYS', 'kaizen.decision', { id: task.id, verdict: d.verdict, tier: d.tier || null, note: task.applyNote || task.rejectReason || '' });
      if (task.priority === 'P1' && task.status !== 'REJECTED') {
        state.alertCount++;
...
// Each cycle: drain duplicate PROPOSED tasks (bounded), and give hands to
// human-ACCEPTED ENV_VAR tasks — clicking ACCEPT on the dashboard now means
// "apply this to Railway for me".
async function brainSweep() {
  let rejects = 0;
  const seen = new Map();
  const chrono = [...state.tasks].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
  for (const t of chrono) {
    const fp = fingerprint(t);
    if (t.status === 'PROPOSED') {
      if (seen.has(fp)) {
        if (rejects < MAX_AUTO_REJECT_PER_CYCLE) {
          t.status = 'REJECTED';
          t.rejectReason = `duplicate of task #${seen.get(fp)}`;
          rejects++;
  ...
  for (const t of state.tasks.filter(x => x.status === 'ACCEPTED' && x.effort === 'ENV_VAR')) {
    const r = await executor.applyRemedy(t, fetch);
    if (r.ok) {
      t.status = 'DEPLOYED';
      t.deployedAt = new Date().toISOString();
      t.applyNote = r.note;
      emit('TASK', 'kaizen.task.update', { id: t.id, status: 'DEPLOYED', target: t.target, title: t.title });
      emit('SYS', 'kaizen.applied', { id: t.id, note: r.note });
    } else if (t.applyNote !== r.note) { t.applyNote = r.note; }
  }
}
...
function setTaskStatus(id, status) {
  ...
  // ACCEPT on an ENV_VAR task = "apply it for me" — executor acts immediately.
  if (t.status === 'ACCEPTED' && t.effort === 'ENV_VAR' && executor.enabled()) {
    executor.applyRemedy(t, fetch).then(r => {
      if (r.ok) {
        t.status = 'DEPLOYED'; t.deployedAt = new Date().toISOString(); t.applyNote = r.note;
        emit('TASK', 'kaizen.task.update', { id: t.id, status: 'DEPLOYED', target: t.target, title: t.title });
        emit('SYS', 'kaizen.applied', { id: t.id, note: r.note });
      } else {
        t.applyNote = r.note;
        emit('SYS', 'kaizen.apply.refused', { id: t.id, note: r.note });
      }
    }).catch(() => {});
  }
  return t;
...
server.listen(PORT,()=>{
  ...
  console.log(`  Law  →  Tier-A auto-applies (${executor.enabled() ? 'EXECUTOR ARMED' : 'no RAILWAY_API_TOKEN — propose-only'}) · locked vars never\n`);
